...
The Cloudflare Tunnel integration tips further toward ZeroClaw for deployments that need WhatsApp or LINE webhooks. For this deployment's current scope (Telegram + Discord, both outbound polling), a tunnel is not required for messaging. It would be useful only for remote dashboard access. If WhatsApp or LINE channels are added later, ZeroClaw's managed tunnel avoids the manual `cloudflared` wiring that PicoClaw would require.
