          ...
          case "$2" in
            connectivity-change)
              ...
              if [ "$CONNECTIVITY_STATE" == "PORTAL" ]; then
                ...
                USER_ID=$(${pkgs.coreutils-full}/bin/id -u "${username}")
                USER_SESSION=$(/run/current-system/sw/bin/loginctl list-sessions --no-legend | ${pkgs.gawk}/bin/awk -v uid="$USER_ID" '$3 == uid {print $1}' | ${pkgs.coreutils-full}/bin/head -n 1)
                ...
                DISPLAY=$(/run/current-system/sw/bin/loginctl show-session "$USER_SESSION" -p Display | ${pkgs.coreutils-full}/bin/cut -d'=' -f 2)
                ...
                TIMEOUT_CMD="${pkgs.coreutils-full}/bin/timeout 30"
