...
EOF
  ...
  echo
  echo "3. Add the public key to GitHub and Configure SSO"
  echo "    https://github.com/settings/keys"
  echo
  echo "4. Switch to the update Home Manager:"
  echo "    just switch-home"
  echo
  echo "5. Add the key to the SSH agent:"
  echo "     ssh-add ~/.ssh/id_ed25519_sk_${KEY}"
  echo "    󰀵 ssh-add --apple-use-keychain ~/.ssh/id_ed25519_sk_${KEY}"
  echo
  echo "6. List the keys to verify:"
  echo "    ssh-add -L"
  echo "    󰈷 the "
  echo
  echo "7. Run the following, if you haven't already:"
