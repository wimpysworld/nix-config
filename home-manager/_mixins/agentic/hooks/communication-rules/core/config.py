...
# ``briefReminderPrompt``. A fresh main-thread context that already holds the
# rules in its system prompt gets this pointer instead of a second copy. A
# sub-agent context, and any platform that carries nothing, still gets
# ``reminder_prompt`` in full.
...
class Config:
    def __init__(
        ...
        house_style_in_system_prompt: dict[str, bool] | None = None,
    ) -> None:
        ...
        # repeating it, for a platform whose system prompt carries the body.
        self.brief_reminder_prompt = brief_reminder_prompt or FALLBACK_BRIEF_REMINDER_PROMPT
        # Which platforms carry the house style in their system prompt, keyed by
        # core agent name. Generated by Nix from each platform's own carriage.
        self.house_style_in_system_prompt = dict(house_style_in_system_prompt or {})
    ...
    def carries_house_style(self, agent: str) -> bool:
        """Whether ``agent`` carries the house style in its system prompt.

        Only an explicit ``True`` from the generated policy counts. An unknown
        agent, a missing flag, and an absent policy all read as False, so the
        reminder falls back to the full rules rather than pointing at a copy
        that may not be there.
        """
        return self.house_style_in_system_prompt.get(agent) is True

    def fresh_context_reminder(self, agent: str) -> str:
        """The reminder text for a fresh MAIN-THREAD context of one agent.

        The brief pointer applies only where the platform carries the house
        style; every other platform gets the full rules. A sub-agent context is
        decided in ``core.dispatch``, which always gives it the full body.
        """
        if self.carries_house_style(agent):
            return self.brief_reminder_prompt
        return self.reminder_prompt
...
def load_house_style_flags(value: object) -> dict[str, bool]:
    """Read the per-agent house-style carriage map from the loaded policy.

    Keeps only genuine booleans, so a malformed entry cannot read as a truthy
    carriage claim and downgrade that agent's reminder to the brief pointer.
    """
    if not isinstance(value, dict):
        return {}
    return {key: flag for key, flag in value.items() if isinstance(key, str) and isinstance(flag, bool)}
...
def load_config(args: argparse.Namespace) -> Config | None:
    ...
    house_style_in_system_prompt: dict[str, bool] = {}
    block_message = None
    ...
    if getattr(args, "policy_json", None):
        ...
        house_style_in_system_prompt = load_house_style_flags(loaded.get("houseStyleInSystemPrompt"))
    ...
    return Config(
        ...
        house_style_in_system_prompt=house_style_in_system_prompt,
    )
