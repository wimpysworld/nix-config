...
Because the system prompt already holds the rules, a fresh main-thread context gets a one-line pointer to the house style instead of a second copy. A Codex sub-agent still gets the full body, since neither an output style nor `developer_instructions` reaches a sub-agent context.
...
- The fresh-context reminder was cut to the one-line pointer on every main thread after the house style landed in each platform's system prompt. The live rows above predate that cut: they confirm a reminder arrives, and the fixture and shim suites confirm which text. Codex `SubagentStart` still carries the full body.
