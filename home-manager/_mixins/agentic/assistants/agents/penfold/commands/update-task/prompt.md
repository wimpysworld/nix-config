...
- At root, assign each missing research question to a bounded worker. In worker mode, research the assigned question directly. Research only as far as the decision needs. Source order: this repo's code and patterns, upstream source, current documentation, then the web.
