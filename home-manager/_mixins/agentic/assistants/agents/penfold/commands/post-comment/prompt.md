...
This command reads from and mutates GitHub, Linear, or Slack over the network and speaks as the user. For GitHub and Slack, it also writes the approved body to a temporary file and runs the commands named below. Treat explicit human invocation of this command as consent for those actions. For GitHub, load `gh` before resolving the target or making any other GitHub access, and follow it for all GitHub tool and policy choices.
...
4. Post with the dedicated tool for that surface:
   ...
   - Linear: the Linear MCP comment tool. Send real newlines in the body, never literal backslash-n. For a Linear document inline comment only, first confirm that the available write tool can create and verify a true inline anchor. If it cannot, do not post an unanchored substitute. Resolve the best manual placement with read-only Linear access and return the manual-placement output below. Keep ordinary Linear issue comments and document comments with supported, verifiable inline anchors on this posting path
...
If the Linear document inline-comment fallback applies, return:
...
Highlight exactly: <shortest exact contiguous passage>
Placement reason: <one sentence>
Comment:
<approved verbatim fenced comment from `draft-comment`>
```
