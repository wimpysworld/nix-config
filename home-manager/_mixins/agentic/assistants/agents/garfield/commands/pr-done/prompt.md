...
6. Load `contribution-voice`. Draft one to three visible sentences that summarise material follow-up and the final result. When none occurred, say that the change merged without a material follow-up. Show no PR URL or number. Append `<!-- pr-done-workflow:<full-head-sha> -->` to the source body.
