...
The user says the pull request they have been working on is merged. Return the repository to a clean state: `main` checked out and up to date, the merged branch gone, and any worktree for it removed.
...
This command deletes a local branch and removes worktrees. Treat explicit human invocation as consent to fetch, to check out and fast-forward `main`, to remove the merged branch's worktrees, and to delete the merged local branch. Nothing else: no force flags on a worktree removal, no `git push` of any kind, no merging, no closing, and no change to any other branch.

Out of scope: this command changes no GitHub or Linear state, and reads none either. Moving the tracking issue to Done is Linear's own automation or a separate step.
...
7. Move to the main worktree, whose path is the first entry in `git worktree list`. Run every remaining Git command as `git -C <main-worktree-path> ...`. A worktree cannot be removed while it is the current directory, and the current directory may be the worktree about to go.
8. Update `main`. Check it out with `git -C <main-worktree-path> checkout main`, then run `git -C <main-worktree-path> pull --ff-only`. If either is refused, stop and report it. A diverged `main` is the user's call to resolve, not this command's.
9. Remove each worktree with `git -C <main-worktree-path> worktree remove <worktree-path>`, then run `git -C <main-worktree-path> worktree prune`. Never pass `--force`. If a removal is refused, step 6 missed something: stop and report it rather than forcing.
10. Delete the local branch with `git -C <main-worktree-path> branch -D <branch>`. `git branch -d` refuses a squash-merged branch, because its commits never appear on `main` as themselves, and a squash merge is the normal case here. What makes `-D` safe here is the user's assertion plus the checks in step 6, not any lookup. Never reach for it on a branch that failed a check.
11. Report, following **Output** below. Carry the step 5 result into the report, and say if the user's shell is still sitting in a worktree that was removed.
12. If the remote-tracking ref survived the prune in step 5, print the notice from **Dangling remote branch** below as the last thing you say. If the prune removed it, say nothing beyond the corroboration line already in the report: no notice, no empty section, no "nothing to do here".
...
The `git push origin --delete` line in step 12 is printed for the user to run, never run by this command. Keep it that way. Never offer a `gh` form of it either: the GitHub CLI has no branch-delete subcommand, and the raw API route is denied.
