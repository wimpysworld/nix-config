...
Read infrastructure code, run available local checks, and write only the
report at the derived path below. Do not change infrastructure, secrets,
workflows, or runtime state.
...
- Write the aggregated report to the derived path, then report that path.
