...
1. Invoke `less` to reload the Communication Rules before starting. Codex uses `$less`; slash-command runtimes use `/less`. If the platform cannot expand a command, apply the rules restated below instead
