...
2. Delegate to a wide fan-out of workers, in parallel where possible. Split by subdirectory, recursing into every nested subdirectory, not only top-level ones. First-party code only: exclude git submodules. Each worker runs this same peer review over its own directory; the parent aggregates the findings
