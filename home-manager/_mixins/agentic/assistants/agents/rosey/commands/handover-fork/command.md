...
- Read `contribution-voice` first unless its complete, current instructions are in this context. Apply it. The section table below fixes the headings, so its rule against scaffolding does not apply to them; it governs the prose inside. Run its cut pass before returning the briefing.
