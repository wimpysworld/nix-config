...
`make-pr` and `finish-pr` take no task argument. They resolve each linked issue from the Git evidence instead, by the same shape rules: a `Refs: <ISSUE-KEY>` trailer is Linear, a `Refs: <owner>/<repo>#<n>` trailer is GitHub, an issue-key token in the branch name at non-alphanumeric boundaries is Linear when its prefix matches a team key visible in the connected workspace, and a pull request's `closingIssuesReferences` entry is GitHub. One branch may link issues in both trackers.
...
| done | Merged. `finish-pr` moves here |
