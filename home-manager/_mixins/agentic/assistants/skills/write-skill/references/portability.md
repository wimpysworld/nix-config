...
Use an agent-backed command for a workflow that needs agent defaults. Put those defaults in the agent's header, not the command or skill. Explicit launch-time child overrides remain supported. Skill invocation and reads never change the caller's model.
