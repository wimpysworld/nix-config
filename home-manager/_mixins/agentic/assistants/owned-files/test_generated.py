...
class GeneratedDeploymentTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory(prefix="generated-owned-files-") as directory:
            ...
            def activate():
                ...
                result = subprocess.run(
                    [sys.executable, str(helper), str(desired)],
                    capture_output=True,
                    text=True,
                )
            ...
            for entry in spec["files"]:
                ...
                with self.subTest(path=path):
                    if entry["kind"] == "file":
                        ...
                        self.assertEqual(
                            path.read_bytes(),
                            entry.get("prefix", "").encode()
                            + Path(entry["source"]).read_bytes(),
                        )
                    else:
            ...
            self.assertEqual(
                {p.name for p in manifest.parent.iterdir()}, {"manifest.json"}
            )
    ...
    @unittest.skipUnless(
        os.environ.get("ASSISTANT_OWNED_SPEC"),
        "Set ASSISTANT_OWNED_SPEC to a built specification.",
    )
