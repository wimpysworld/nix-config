...
Keep it cheap. List the directory names under `agents/`, `commands/`, `agents/*/commands/`, and `skills/`. The names are descriptive, so shortlist from them, then read `[common] description` in `command.toml` for commands or `header.toml` for agents and skills for the few candidates that matter. Do not read `README.md`. Do not read every prompt: the tree holds dozens of commands, and this review does not warrant that cost.
