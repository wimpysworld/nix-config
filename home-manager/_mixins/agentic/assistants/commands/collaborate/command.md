...
Run this command in the current context. Its bounded read-only source ingestion is an explicit exception to the global requirement to delegate non-trivial work. Do not launch a worker or a Task for any step. This exception grants no mutation authority. The purpose is to prime current context.
