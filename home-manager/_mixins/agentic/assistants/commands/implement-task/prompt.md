...
5. After validation passes, run `update-docs <changed files>` where the change requires documentation. If prompt, command, skill, assistant, or project instruction artefacts changed, run the matching Rosey workflow: `update-command`, `update-skill`, `update-assistant`, or `update-agents-md`.
