...
6. Write the durable record: 2 to 4 sentences of prose saying what now exists and anything a reader must act on. No headings, no bullets, no file list. Load the `contribution-voice` skill and follow it before writing; the record publishes under the user's name. State the work as built, not the plan to build it. Include only what a reader cannot get from the diff or the issue: a decision that went against the obvious approach, and a risk carried forward. Say the acceptance criteria pass in one clause, and name a criterion only when it does not. Never walk the criteria one by one and never restate the task. Post it as a comment on the Linear issue, or append it to the local task file.
