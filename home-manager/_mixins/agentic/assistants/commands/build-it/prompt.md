...
9. Enumerate the plan's implementation units before writing code: every phase, sub-phase, and task with its dependencies, scope, owner, and success criteria.
10. Implement one unit at a time through a fresh sub-agent or fresh command invocation for that exact unit. Use `implement-plan <plan> <unit-id>` where the provider supports command expansion. Never pass the whole plan, a whole phase, or multiple unrelated units to one sub-agent. Run independent units in parallel only when their dependencies are satisfied and each unit still gets its own fresh sub-agent.
11. If the platform cannot expand a command from inside this command, perform the same split directly: delegate each phase, sub-phase, or task as its own bounded packet, with fresh context and only the context needed for that unit.
12. Run `validate-plan <plan> <proposal>` with the changed files or implementation diff. Fix required gaps and rerun validation until it passes or a real blocker remains.
13. After validation passes, run `update-docs <changed files or implementation diff>` using the provider-prefix rule above. This delegates documentation checks and required updates to Velma. Update documentation only where the implemented change requires it.
14. If prompt, command, skill, assistant, or project instruction artefacts changed, delegate their documentation checks and required updates to Rosey. Run the relevant Rosey workflow, such as `update-command`, `update-skill`, `update-assistant`, or `update-agents-md`, using the provider-prefix rule above.
15. Review the working tree. Stage only code and documentation intended for commit. Do not stage working documents or planning artefacts, including overview, proposal, plan, alignment, validation, research, decision documents, or other build artefacts. Use explicit path-limited staging.
16. Run `draft-commit-message` using the provider-prefix rule above. Preserve its fenced commit message unchanged for the final output. Never create a commit.

If the platform cannot expand a command from inside this command, perform the same workflow directly while preserving the per-unit implementation split. Use the current command catalogue and project instructions as the source of truth.
...
- During implementation, never hand an entire multi-phase plan to one sub-agent. Split by phase, sub-phase, and task, then use one fresh sub-agent per unit.
