...
The server exists only on hosts tagged `workspace`. Claude Code, Codex, OpenCode, and Pi enable it. Zed stays disabled. Claude Code and Codex remove `slack_send_message`, `slack_send_message_draft`, `slack_schedule_message`, and `slack_update_canvas` from their tool lists. Slack reads, searches, profiles, files, and reactions remain available. Messages use `slack-post` so Slack attributes them to the user.
