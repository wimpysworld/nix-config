...
let
  ...
  # Neocode aliases for launching the modeless Neovim configuration
  shellAliases = {
    ncode = "nvim";
    neocode = "nvim";
  };
in
{
      ...
      extraLuaConfig = lib.mkBefore ''
        ...
        require('snacks').setup {
          ...
          dashboard = {
            ...
            sections = {
              ...
              {
                text = { { ":+:+:   :+: :+:       :+:    :+: :+:    :+: :+:    :+: :+:    :+: :+:        ", hl = "SnacksDashboardBannerPeach" } },
                align = "center",
              },
              {
                text = { { ":+:+:+  +:+ +:+       +:+    +:+ +:+        +:+    +:+ +:+    +:+ +:+        ", hl = "SnacksDashboardBannerYellow" } },
                align = "center",
              },
              {
                text = { { "+#+ +:+ +#+ +#++:++#  +#+    +:+ +#+        +#+    +:+ +#+    +:+ +#++:++#   ", hl = "SnacksDashboardBannerGreen" } },
                align = "center",
              },
              {
                text = { { "+#+  +#+#+# +#+       +#+    +#+ +#+        +#+    +#+ +#+    +#+ +#+        ", hl = "SnacksDashboardBannerSky" } },
                align = "center",
              },
              {
                text = { { "#+#   #+#+# #+#       #+#    #+# #+#    #+# #+#    #+# #+#    #+# #+#        ", hl = "SnacksDashboardBannerBlue" } },
                align = "center",
              },
              {
                text = { { "###    #### ########## ########   ########   ########  #########  ########## ", hl = "SnacksDashboardBannerMauve" } },
                align = "center",
              },
              -- Tagline
              { padding = 0 },
              ...
              },
              {
                text = {
                  ...
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "        ", hl = "SnacksDashboardKey" }, { "          ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
              ...
              {
                text = {
                  { "Ctrl+N  ", hl = "SnacksDashboardKey" }, { "New File  ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Ctrl+S  ", hl = "SnacksDashboardKey" }, { "Save      ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Alt+S   ", hl = "SnacksDashboardKey" }, { "Save As   ", hl = "SnacksDashboardDesc" },
                },
                align = "center",
              },
              {
                text = {
                  { "Ctrl+B  ", hl = "SnacksDashboardKey" }, { "Explorer  ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Ctrl+`  ", hl = "SnacksDashboardKey" }, { "Terminal  ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Ctrl+Q  ", hl = "SnacksDashboardKey" }, { "Quit      ", hl = "SnacksDashboardDesc" },
                },
                align = "center",
              },
              -- Separator
              ...
              {
                text = {
                  { "Ctrl+Alt+I", hl = "SnacksDashboardKey" }, { "  Chat   ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Alt+M   ", hl = "SnacksDashboardKey" }, { "Model     ", hl = "SnacksDashboardDesc" },
                  { "    ", hl = "SnacksDashboardDesc" },
                  { "Alt+A   ", hl = "SnacksDashboardKey" }, { "Actions   ", hl = "SnacksDashboardDesc" },
                },
                align = "center",
                padding = 1,
              },
        ...
        vim.api.nvim_create_autocmd("ColorScheme", {
          callback = function()
            ...
            -- Rainbow banner highlight groups
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerRed", { fg = "${catppuccinPalette.getColor "red"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerPeach", { fg = "${catppuccinPalette.getColor "peach"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerYellow", { fg = "${catppuccinPalette.getColor "yellow"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerGreen", { fg = "${catppuccinPalette.getColor "green"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerSky", { fg = "${catppuccinPalette.getColor "sky"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerBlue", { fg = "${catppuccinPalette.getColor "blue"}", bold = true })
            vim.api.nvim_set_hl(0, "SnacksDashboardBannerMauve", { fg = "${catppuccinPalette.getColor "mauve"}", bold = true })
        ...
        -- Shift+Ins to paste from system clipboard (same as Ctrl+V)
        -- Remap to Ctrl+V so novim-mode handles it properly (including selection replacement)
        vim.keymap.set({'n', 'i', 'v', 's'}, '<S-Ins>', '<C-v>', { remap = true, silent = true, desc = 'Paste from clipboard' })

        -- Smart Shift+Home: select to first non-whitespace, or to column 1 if already there
        -- Mimics VSCode/modern editor behaviour for smart line-start selection
        -- IMPORTANT: Works WITH novim-mode's selection model, not against it
        -- - novim-mode handles Ctrl+C/X/V in select mode via its own snoremap mappings
        -- - We only override Shift+Home behaviour, using novim-mode's BehaveMSWin() for consistency
        vim.api.nvim_create_autocmd('VimEnter', {
          callback = function()
            -- Helper: determine smart home target (^ or 0)
            local function smart_home_target()
              local col = vim.fn.col('.')
              local match_pos = vim.fn.match(vim.fn.getline('.'), '\\S')

              -- If line is whitespace-only (match returns -1), toggle between column 1 and EOL
              if match_pos == -1 then
                if col == 1 then
                  return '$'  -- Go to end of line
                else
                  return '0'  -- Go to column 1
                end
              end

              -- Normal case: toggle between first non-blank and column 1
              local first_non_blank = match_pos + 1  -- Convert to 1-indexed
              return (col == first_non_blank) and '0' or '^'
            end

            -- Insert mode: use novim-mode's selection model (BehaveMSWin + select mode)
            vim.keymap.set('i', '<S-Home>', function()
              -- Set up CUA selection behaviour (like novim-mode does)
              vim.fn['g:BehaveMSWin']()
              local target = smart_home_target()
              -- Start selection and move to target (enters select mode due to BehaveMSWin)
              local keys = vim.api.nvim_replace_termcodes('<S-Left><C-g><C-o>' .. target, true, false, true)
              vim.api.nvim_feedkeys(keys, 'n', false)
            end, { noremap = true, silent = true, desc = 'Smart select to line start' })

            -- Select mode: extend selection to target (simple, like novim-mode's other snoremap bindings)
            vim.keymap.set('s', '<S-Home>', function()
              local target = smart_home_target()
              -- <C-o> allows one normal mode command while keeping selection
              local keys = vim.api.nvim_replace_termcodes('<C-o>' .. target, true, false, true)
              vim.api.nvim_feedkeys(keys, 'n', false)
            end, { noremap = true, silent = true, desc = 'Smart extend selection to line start' })

            -- Smart Home (no selection): toggle between first non-whitespace and column 1
            -- Mimics VSCode/modern editor behaviour
            vim.keymap.set('n', '<Home>', function()
              local target = smart_home_target()
              vim.cmd('normal! ' .. target)
            end, { noremap = true, silent = true, desc = 'Smart home' })
  ...
  programs = {
    ...
    '';
    # Neocode shell aliases
    bash.shellAliases = lib.mkIf config.programs.neovim.enable shellAliases;
    fish.shellAliases = lib.mkIf config.programs.neovim.enable shellAliases;
    zsh.shellAliases = lib.mkIf config.programs.neovim.enable shellAliases;
  };
