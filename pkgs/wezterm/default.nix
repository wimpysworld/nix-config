...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  postPatch = ''
    ...
    # The hash command does not work well with NixOS.
  ...
  # The dep: syntax causes build failures in rare cases.
  ...
  nativeBuildInputs = [
    installShellFiles
    ncurses # Provide tic for terminfo.
    pkg-config
    python3
  ]
  ...
  preFixup =
    ...
    + lib.optionalString stdenv.hostPlatform.isDarwin ''
      ...
      # macOS only recognises the application bundle when it contains the
      # binaries. Move them there
  ...
  passthru = {
    # The headless variant is useful when deploying WezTerm's mux server on remote servers.
