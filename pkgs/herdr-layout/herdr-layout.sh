...
fail_workspace() {
	printf 'herdr-layout: workspace %s: %s. The partial workspace was preserved. Run herdr workspace focus %s and finish the layout manually.\n' \
...
# Variant: @INITIAL_LABEL@ layout. Labels without a command keep plain shells.
...
if ((review_enabled)) && [[ -n $worktree_checkout ]] &&
	[[ $(basename -- "$worktree_checkout") == *review-* ]] &&
...
for label in @EXTRA_PANES@; do
	create_tab "$label"
	pane_ids["$label"]=$created_pane_id
done

for label in "$initial_label" @EXTRA_PANES@; do
	command_text=${agent_commands[$label]:-}
	[[ -n $command_text ]] || continue
	if [[ $label == "$initial_label" ]]; then
		run_in_pane "$initial_pane_id" "$command_text"
	else
		run_in_pane "${pane_ids[$label]}" "$command_text"
	fi
done

herdr_call response "focusing the $initial_label tab" tab focus "$initial_tab_id"
validate_focused_tab_info "$response" "$initial_tab_id" "$initial_label" "focusing the $initial_label tab"

printf 'herdr-layout: prepared workspace %s and focused tab %s.\n' "$workspace_id" "$initial_tab_id"
